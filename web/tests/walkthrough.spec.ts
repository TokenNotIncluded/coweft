...
// Actual interactions against the compiled website with explicit API fixtures.
...
test('record actual navigation, AI tools and editing', async ({ page }, info) => {
  ...
  await page.getByRole('button', { name: '列表视图' }).click(); await page.waitForTimeout(650);
  await page.getByRole('button', { name: '空间视图' }).click(); await page.waitForTimeout(650);
  ...
  await page.getByRole('link', { name: '在完整页面打开' }).click();
