...
test.describe('recorded browser walkthrough', () => {
  test.use({ video: { mode: 'on', size: { width: 1280, height: 900 } } });
  test('real frontend navigation, AI tools and editing', async ({ page }, info) => {
    test.skip(info.project.name !== 'desktop', 'One desktop walkthrough is enough; mobile is covered above.');
    await mockCommunity(page); await page.goto('/');
