...
export function Settings({ open, close }: { open: boolean; close: () => void }) {
  ...
  return <Modal open={open} onOpenChange={value => { if (!value) close(); }} layout="sheet" title="一个身份。两个搭档。" description="登录、授权与撤销，由 api.lmm.best 统一处理。">
    ...
    <div className="settings-bottom"><p>同一份贡献记录。<br/>同一份治理资格。</p>{me.data && <Button variant="ghost" onClick={() => logout.mutate()} disabled={logout.isPending}><LogOut size={15}/>{logout.isPending ? '退出中…' : '退出会话'}</Button>}</div>
  </Modal>;
}
