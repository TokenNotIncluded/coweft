...
export function ThreadPage() {
  const { id } = useParams();
  const query = useQuery({ queryKey: ['thread', id], queryFn: ({ signal }) => api<Detail>(`/api/threads/${id}`, { signal }), enabled: !!id });
  ...
  return <ThreadBody key={query.data.thread.id} data={query.data} refresh={() => { void query.refetch(); }}/>;
}

function ThreadBody({ data, refresh }: { data: Detail; refresh: () => void }) {
  ...
  return <article className="page thread-page">
    <div className="thread-breadcrumb"><Link className="back-link" to="/"><ArrowLeft size={15}/>所有讨论</Link><span className="mono">THREAD / {thread.id.slice(0, 8)}</span></div>
    ...
    <Modal open={proposing} onOpenChange={setProposing} layout="editor" title="让讨论，进入一个决定。" description="讨论与表决持续 3 天，按发起时的成员快照计票。每个共同账号一票。"><form className="compose" onSubmit={event => { event.preventDefault(); const fields = new FormData(event.currentTarget); if (!command.isPending) command.mutate({ action: 'propose', thread_id: thread.id, title: String(fields.get('title') ?? ''), rationale: String(fields.get('rationale') ?? '') }); }}><label>提案标题<input name="title" maxLength={180} required placeholder="具体要改变什么？"/></label><label>方案、影响与反对意见<textarea name="rationale" maxLength={12000} required rows={9} defaultValue={model.variables === 'proposal' ? model.data?.content ?? '' : ''} placeholder="写出备选方案、受影响的人和可逆的试行方式。"/></label><ErrorNotice error={command.error}/><div className="form-actions"><span className="muted small">通过表决决定行动，不决定事实真伪。</span><Button disabled={command.isPending}>发布提案 <ArrowUpRight size={15}/></Button></div></form></Modal>
    <Modal open={addingEvidence} onOpenChange={setAddingEvidence} title="留下一条可核查的证据。" description="贡献记录不产生等级，也不增加投票权。"><form className="compose" onSubmit={event => { event.preventDefault(); const fields = new FormData(event.currentTarget); const kind = fields.get('kind'); if (!command.isPending && (kind === 'reproduced' || kind === 'correction' || kind === 'useful')) command.mutate({ action: 'evidence', thread_id: thread.id, kind, note: String(fields.get('note') ?? '') }); }}><label>类型<select name="kind"><option value="reproduced">我已复现</option><option value="correction">补充纠正记录</option><option value="useful">对我有帮助</option></select></label><label>说明与来源<textarea name="note" rows={5} required maxLength={2000} placeholder="具体做了什么？其他人可以如何核实？"/></label><ErrorNotice error={command.error}/><Button disabled={command.isPending}>记录证据 <ArrowUpRight size={15}/></Button></form></Modal>
  </article>;
}
